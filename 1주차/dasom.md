...
* 연산자: 하나 이상의 표현식을 대상으로 산술, 할당, 비교, 논리, 타입, 지수 연산 등을 수행
* 피연산자 + 연산자 -> 연산자 표현식 (값으로 평가됨)
...
* 할당 연산자
  ...
  * = 외에도 이항 산술연산자와 =을 함께 축약형으로 사용

* 비교 연산자

  * == 동등 비교: 암묵적 타입 변환을 수행 / 부동등 !=
  * === 일치 비교 / !== 불일치 비교

  cf. NaN은 자기 자신과 일치 하지 않는 유일한 값
...
* 쉼표 연산자
  ...
  * 왼쪽 피연산자부터 차례대로 피연산자를 평가, 마지막 피연산자의 평가 결과 반환
...
## 느낀점
 ...
 제 5장 표현문과 문 부분에서 저자는 "용어의 의미를 정확히 설명할 수 없다면 개념을 제대로 이해하지 못한 경우가 많다." 라는 말을 했는데, 앞 부분을 여러번 읽은 시점에서 더더욱 와닿는 말이었다. 

 분명히 읽는 것과 설명할 수 있는 것은 다르다. 발표자가 아니라면 "읽는 것"에 초점을 맞추기 쉬운데 어떤 상황이든 상관 없이 "설명할 수 있는 것"에 초점을 맞춰서 스터디를 진행해야겠다.



**오늘의 간단 요약**

변수란? 메모리에 저장된 값을 쉽게 찾아줄 수 있는 식별자

표현식인 문과 표현식이 아닌 문의 차이? 변수에 할당을 해보자! 단, 선언문 자체는 표현식인 문이다.
